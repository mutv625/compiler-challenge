...
class Program
{
    static void Main()
    {
        Console.WriteLine("Input directory name (e.g., SimpleAdd): ");

        string inpurDirName = Console.ReadLine() ?? throw new ArgumentException("Input directory name is required.");

        string inputDirPath = $@"../../../IOFiles/{inpurDirName}";
        string outputPath = $@"../../../IOFiles/{inpurDirName}/{inpurDirName}.asm";

        // TODO: 8.6 フォルダー内の複数ファイルを一つの.asmに翻訳する処理
        // * 1. 入力ファイル群を取得
        string[] vmFiles = Directory.GetFiles(inputDirPath, "*.vm");
        ...
        {
            // * DEBUG
            Console.WriteLine($"Translating {vmFile} to {inputDirPath}/_{Path.GetFileNameWithoutExtension(vmFile)}.asm");

            var translator = new VMTranslator(vmFile, $"{inputDirPath}/_{Path.GetFileNameWithoutExtension(vmFile)}.asm");
            translator.TranslateAndOutput();
        }


        // * 3. 出力ファイルを結合して最終出力ファイルを作成
        using (var finalWriter = new StreamWriter(outputPath))
        {
            // 3.1 _sys.asm と ブートストラップを最初に結合
            string sysFilePath = $"{inputDirPath}/_Sys.asm";

            if (File.Exists(sysFilePath))
            {
                // * DEBUG
                Console.WriteLine($"Initial merging {sysFilePath} to final output.");
            ...
            // 3.2 _Main.asm を結合
            string mainFilePath = $"{inputDirPath}/_Main.asm";
            
            if (File.Exists(mainFilePath))
            {
                // * DEBUG
                Console.WriteLine($"Initial merging {mainFilePath} to final output.");
            ...
            foreach (string vmFile in vmFiles)
            {
                string asmFilePath = $"{inputDirPath}/_{Path.GetFileNameWithoutExtension(vmFile)}.asm";

                // _Sys.asm ,_Main.asm はスキップ
                if (asmFilePath == sysFilePath || asmFilePath == mainFilePath)
                {
                    continue;
                }

                // * DEBUG
                Console.WriteLine($"Merging {asmFilePath} to final output.");
...
class VMTranslator
{
    ...
    /// TranslateAndOutput() で翻訳と出力ファイルの上書きを行います。
    /// </summary>
    /// <param name="inputFilePath">VMファイルパス</param>
    /// <param name="outputFilePath">ASMファイルの出力先、出力ファイル名</param>
    public VMTranslator(string inputFilePath, string outputFilePath)
    {
    ...
    public void TranslateAndOutput()
    {
...
class Parser
{
    ...
    public Parser(StreamReader vmCodeReader)
    {
        reader = vmCodeReader;
    }
